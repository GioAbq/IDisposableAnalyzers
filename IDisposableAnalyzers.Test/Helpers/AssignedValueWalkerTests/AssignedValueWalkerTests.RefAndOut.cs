...
namespace N
{
    public static class C
    {
        ...
        private static bool TryGet(out int i)
        {
            ...
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("1", string.Join(", ", walker.Values));
        }
...
namespace N
{
    ...
    public static class C
    {
        ...
        private static bool TryGet(int i, out int result)
        {
            ...
            var value = syntaxTree.FindArgument(expression).Expression;
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("result, 1", string.Join(", ", walker.Values));
        }
...
namespace N
{
    ...
    public static class C
    {
        ...
        private static bool TryGet(int i, out int result)
        {
            ...
            Assert.Equal(true, semanticModel.TryGetSymbol(value, CancellationToken.None, out var parameter));
...
namespace N
{
    ...
    class M
    {
        ...
        [InlineData("var temp3 = value;", "")]
        [InlineData("var temp4 = value;", "2")]
...
namespace N
{
    internal class C<T>
    {
        ...
        internal void Assign(out T outValue)
        {
            ...
            var value = syntaxTree.FindEqualsValueClause("var temp = value;").Value;
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("default(T)", walker.Values.Single().ToString());
        }

        [Theory]
        [InlineData("var temp1 = value;", "0")]
        [InlineData("var temp2 = value;", "0, 1")]
...
namespace N
{
    internal class C
    {
        ...
        [InlineData("var temp3 = this.value;", "1, 2, 3")]
...
namespace N
{
    internal class C
    {
        ...
        internal void Assign(ref int refValue, out int outValue)
        {
            ...
            var value = syntaxTree.FindEqualsValueClause("var temp = value").Value;
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("0, 1, 2", string.Join(", ", walker.Values));
        }
...
namespace N
{
    internal class C
    {
        ...
        private static bool M(out int result)
        {
            ...
            var value = syntaxTree.FindEqualsValueClause("var temp = value").Value;
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("0, result, 1", string.Join(", ", walker.Values));
        }
...
namespace N
{
    internal class C
    {
        ...
        private static bool M(double foo, out int result)
        {
            ...
            var value = syntaxTree.FindEqualsValueClause("var temp = value").Value;
            using var walker = AssignedValueWalker.Borrow(value, semanticModel, CancellationToken.None);
            Assert.Equal("0, result", string.Join(", ", walker.Values));
        }
    }
}
